...
*/
p5.prototype.AsciiArt.prototype.convert2dArrayToString = function (_arr2d) {
...
p5.prototype.AsciiArt.prototype.createWeightTable = function () {
    ...
    for (var i = this.__range.min; i <= this.__range.max; i++) {
        ...
        temp_weightTable[i - this.__range.min] = {
            code: i,
            weight: temp_weight,
        };
    }
    ...
    do {
        ...
        for (var i = 0; i < temp_weightTable.length; i++) {
            if (temp_weightTable[i].weight >= 0) {
                if (
                    temp_weight < 0 ||
                    temp_weightTable[i].weight < temp_weight
                ) {
...
p5.prototype.AsciiArt.prototype.convert = function (_image, _w, _h) {
  ...
  */
    if (arguments.length === 3) {
        ...
        _w = Math.floor(Math.abs(_w));
        _h = Math.floor(Math.abs(_h));
        if (_w < 1) _w = 1;
        if (_h < 1) _h = 1;
...
p5.prototype.AsciiArt.prototype.__convert = function () {
    ...
    var temp_result = this.createArray2d(
        this.__graphics.width,
        this.__graphics.height
    );
    ...
    for (var temp_y = 0; temp_y < this.__graphics.height; temp_y++) {
        for (var temp_x = 0; temp_x < this.__graphics.width; temp_x++) {
            ...
            if (this.invertBrightnessFlag)
                temp_weight = temp_range - temp_weight;
            temp_result[temp_x][temp_y] = this.__sketch.char(
                this.__weightTable[temp_weight].code
            );
        }
    }
